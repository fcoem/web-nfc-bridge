# ndef-write-profile Specification

## Purpose

定義產品第一版 NDEF 寫卡格式、`application/json` payload 規則，以及 Connector 的安全寫卡邊界。
...
- **AND** MAY include a bounded `content` JSON object for ERP demo data
